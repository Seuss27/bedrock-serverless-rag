...
| **MW** | **Make it work** — the first successful `destroy → apply → verify` cycle | **F51**, F39, F5, F46, F55 | **in progress** (Tasks 1–4 done; Task 5 drafted, pending its human `bootstrap/` apply; Task 6 remains) |
...
- **`MW` immediately after ST**, and this is the correction BR-D23 exists to make. BR-D20
  ...
  *inside* `MW`, as Task 5** — see the hazard list below, where ordering hazard 1 was
  rewritten to match. `MW` is now in progress with F55 still open by design, not blocked on
  it.
